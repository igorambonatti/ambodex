...
const ModalInputCoin = forwardRef<ModalRef, ModalSelectCoinProps>(
  ({ options }, ref) => {
    const { control, watch } = useForm();
    const { setTokens, fetchBalances, balances } = useCreateSwap();
    ...
    return (
      ...
      >
        <div className="flex w-[480px] flex-col">
          ...
          <span className="mb-3 px-6 text-white">Your Tokens:</span>
          <div className="flex h-[350px] flex-col overflow-y-auto">
            {filteredOptions.map((option) => (
              <CoinOption
                option={{ ...option, balance: balances[option.symbol] }}
                key={option.address}
...
ModalInputCoin.displayName = 'ModalInputCoin';

export default ModalInputCoin;
